	...
	const sva::MotionVecd& A_0)
{
	...
	for(std::size_t i = 0; i < joints.size(); ++i)
	{
		...
		// mbc.bodyAccW[succ[i]] = E_0_i*mbc.bodyAccB[succ[i]];
		// mbc.bodyAccW[succ[i]] = mbc.bodyPosW[succ[i]].invMul(mbc.bodyAccB[succ[i]]);
		mbc.bodyAccW[succ[i]] = E_0_i.invMul(mbc.bodyAccB[succ[i]]);
	}
}
